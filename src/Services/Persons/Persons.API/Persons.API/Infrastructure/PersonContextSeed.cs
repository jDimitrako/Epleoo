...
namespace Persons.API.Infrastructure;


public class PersonContextSeed
{
    public async Task SeedAsync(PersonDbContext context, IWebHostEnvironment env, IOptions<PrSettings> settings, ILogger<PersonContextSeed> logger)
    {
    ...
    */
    ...
    private  AsyncRetryPolicy CreatePolicy(ILogger<PersonContextSeed> logger, string personsefix, int retries = 3)
    {
        return Policy.Handle<SqlException>().
            WaitAndRetryAsync(
                ...
                {
                    logger.LogWarning(exception, "[{personsefix}] Exception {ExceptionType} with message {Message} detected on attempt {retry} of {retries}", personsefix, exception.GetType().Name, exception.Message, retry, retries);
                }
            );
    }
}
